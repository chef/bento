...
variable "iso_checksum" {
  type    = string
  default = "8c3bd1ff3d88b5599147626fa2616d85edcc861ec00edd6863b81dbfb135874c"
}

variable "iso_name" {
  type    = string
  default = "AlmaLinux-8.6-x86_64-dvd.iso"
}
...
locals {
  ...
  build_timestamp = "${legacy_isotime("202205111847")}"
...
build {
  ...
  provisioner "shell" {
    ...
    scripts           = ["${path.root}/scripts/update.sh", "${path.root}/scripts/networking.sh", "${path.root}/../_common/motd.sh", "${path.root}/../_common/sshd.sh", "${path.root}/../_common/vagrant.sh", "${path.root}/../_common/virtualbox.sh", "${path.root}/../_common/vmware.sh", "${path.root}/../_common/parallels.sh", "${path.root}/scripts/cleanup.sh", "${path.root}/../_common/minimize.sh"]
  }
