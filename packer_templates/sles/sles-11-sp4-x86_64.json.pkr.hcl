...
variable "autoinst_cfg" {
  type    = string
  default = "sles-11-sp4-x86_64-autoinst.xml"
}
...
variable "iso_checksum" {
  type    = string
  default = "4fbf22201876d571c719328f385d87d6d690fb3ceb5e093108cdf0373c43e855"
}

variable "iso_name" {
  type    = string
  default = "SLES-11-SP4-DVD-x86_64-GM-DVD1.iso"
}
...
source "vmware-iso" "autogenerated_4" {
  boot_command        = ["<esc><enter><wait>", "linux netdevice=eth0 netsetup=dhcp install=cd:/<wait>", " lang=en_US autoyast=http://{{ .HTTPIP }}:{{ .HTTPPort }}/${var.autoinst_cfg}", " textmode=1<wait>", "<enter><wait>"]
  boot_wait           = "5s"
  cpus                = "${var.cpus}"
  disk_size           = "${var.disk_size}"
...
build {
  ...
  provisioner "shell" {
    ...
    scripts           = ["${path.root}/../_common/motd.sh", "${path.root}/../_common/sshd.sh", "${path.root}/../_common/vagrant.sh", "${path.root}/../_common/virtualbox.sh", "${path.root}/../_common/vmware.sh", "${path.root}/../_common/parallels.sh", "${path.root}/scripts/sudoers.sh", "${path.root}/scripts/zypper-locks.sh", "${path.root}/scripts/remove-dvd-source.sh", "${path.root}/scripts/cleanup.sh", "${path.root}/../_common/minimize.sh"]
  }
