...
variable "iso_checksum" {
  type    = string
  default = "55dbf904b35969be9f6d96f042b7470d1a3cb5c953bba47ab8f79740560d8a8d"
}
...
variable "mirror" {
  type    = string
  default = "http://download.rockylinux.org/pub/rocky"
}
...
locals {
  ...
  build_timestamp = "${legacy_isotime("202207180452")}"
...
build {
  ...
  provisioner "shell" {
    ...
    scripts           = ["${path.root}/scripts/update.sh", "${path.root}/../_common/motd.sh", "${path.root}/../_common/sshd.sh", "${path.root}/scripts/networking.sh", "${path.root}/../_common/vagrant.sh", "${path.root}/../_common/virtualbox.sh", "${path.root}/../_common/vmware.sh", "${path.root}/../_common/parallels.sh", "${path.root}/scripts/cleanup.sh", "${path.root}/../_common/minimize.sh"]
  }
