...
boot_command            = ["<wait><up>e<wait><down><down><end><wait> inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/rhel/9ks.cfg inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/rhel/9ks.cfg inst.repo=https://mirror.stream.centos.org/9-stream/BaseOS/aarch64/os/ <leftCtrlOn>x<leftCtrlOff>"]
